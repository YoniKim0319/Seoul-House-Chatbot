...
X_smote, y_smote = resample_data(X, y)

# 훈련 데이터와 테스트 데이터 분할
X_train, X_test, y_train, y_test = train_test_split(
    X_smote, y_smote, stratify=y_smote, test_size=0.2, random_state=42)

# 랜덤 포레스트 분류기 훈련 및 교차 검증
...
forest_clf = train_model(X_train, y_train)

# 교차 검증을 통한 정확도 측정
forest_clf_cv = cross_val_score(forest_clf, X_train, y_train, cv=5, scoring='accuracy')

# 정확도를 백분율로 변환하여 출력
accuracy_percentage = forest_clf_cv.mean() * 100
...
if st.button('예측 결과 확인'):
    ...
    st.write(f'교차 검증 평균 정확도: {accuracy_percentage:.2f}%')