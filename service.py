...
model_filename = 'satisfaction_model.joblib'
model_url = 'https://drive.google.com/uc?export=download&id=1l6jcZXjwhdUa2AvczkZLdNJ8ppsCKvxC'
...
if not os.path.exists(model_filename):
    with st.spinner('모델 다운로드 중...'):
        ...
        if response.status_code == 200:
            ...
            st.error('모델 다운로드 실패. 서버 응답 코드: {}'.format(response.status_code))
            st.stop()
